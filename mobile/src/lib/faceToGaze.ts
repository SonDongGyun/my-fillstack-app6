...
export function faceToGaze(face: FaceFeature): FaceToGazeResult {
  ...
  const relEyeX = bounds.size.width > 0 ? (eyeX - cx) / bounds.size.width : 0;
  const relEyeY = bounds.size.height > 0 ? (eyeY - cy) / bounds.size.height : 0;
  ...
  // Combine angle + geometric cues; slightly de-weight roll for vertical stability.
  const x = clamp(yaw * 0.62 + noseRelX * 0.82 + relEyeX * 0.74, -0.95, 0.95);
  const y = clamp(relEyeY * 0.95 + noseRelY * 0.58 + roll * 0.08, -0.95, 0.95);
  ...
  const sizeScore = clamp(bounds.size.width / 180, 0.2, 1);
  const symmetryScore = leftEye && rightEye ? clamp(1 - Math.abs(leftEye.y - rightEye.y) / Math.max(1, eyeDistance), 0.25, 1) : 0.45;

  const confidence = clamp(eyeOpenScore * 0.5 + sizeScore * 0.3 + symmetryScore * 0.2, 0.12, 1);
